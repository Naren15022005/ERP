...
@Component({
  selector: 'app-sales-month-widget',
  standalone: true,
  ...
  template: `
    <div class="w-full h-full">
      ...
      @if (loading) {
        ...
        <div class="text-xs text-base-content/60 mt-2">{{ transactions }} transacciones este mes</div>
      }
    </div>
  `
})
export class SalesMonthWidgetComponent implements OnInit {
  ...
  private loadData() {
    this.http.get<any>(`${environment.apiUrl}/dashboard/widgets/sales-month`).subscribe({
      next: (data) => {
